...
//    $("#radio6").button({ icons: {primary:'ui-icon-help'} });
    ...
    methodmap["map"] = "enableMapButtons()";
...
function init() {
    ...
    if (admin == 1) {
        $("#topButtonDiv").append('<input type="radio" id="radio7" name="radiomenu" /><label for="radio7">Administration</label>')
        $("#radio7").button({ icons:{primary:'ui-icon-person'} }).click(function () {
...
function fillMemberListForScripting() {
    var data = {cluster:activeCluster, operation:"memberlist", curtime:0}
    ...
    request.done(function (resp) {
        for (var oper in resp) {
            eval("fillmemberchecklist" + "(resp[oper])")
        }
    });
}
...
function fillmemberchecklist(olist) {
    $('#memberCheckList').empty()
    for (var i = 0; i < olist.length; i++) {
        $('#memberCheckList').append("<div class='memberCheckDiv' ><input id='memberchecklist" + i + "' checked='checked' type='checkbox' name='memberchecklist' value='" + olist[i].label + "'  /><label for='memberchecklist" + i + "' class='itemLink'>" + olist[i].label + "</label></div>")
    }
}
...
function chartmap1(datalist) {
    ...
    var label = $("[name='chartMap1radiogroup']:checked").next().text();
    $.plot($("#chartMap1"), [
...
<div id="mapContentTemplate" class="contentDiv" style="display: none;">
    ...
    <table>
        ...
        <tr>
            <td colspan="3">
                <table id="datatableMap1" class="datatable"></table>
            </td>
        </tr>
        <tr>
            <td colspan="3">
...
<div id="consoleDiv" title="Console">
    <textarea id="commandOutput" readonly="true" rows="14" cols="70"
              style="width: 650px; background-color: #000000; color: #ffffff; font-weight: bold; ">&gt; Type help for
        command list</textarea>
    <br/>
